...
const API_BASE = process.env.NEXT_PUBLIC_CONFIG_API_URL || 'https://call-agent-backend-ssrw.onrender.com'
      ...
      <div className="space-y-6 sm:space-y-8 p-4 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:space-x-4">
          ...
          <div>
            ...
            <p className="text-slate-400 mt-1 text-sm sm:text-base">Customize your AI agent's behavior and responses</p>
          </div>
        </div>

        {/* Main Configuration Card */}
        <div className="bg-slate-900 rounded-xl sm:rounded-2xl border border-slate-800 shadow-xl">
          {/* Tabs */}
          <div className="flex overflow-x-auto border-b border-slate-800">
            {tabs.map((tab) => (
                <button
                    ...
                    className={`
                flex items-center space-x-2 px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm font-medium transition-all whitespace-nowrap
                ${activeTab === tab.id
          ...
          <div className="p-4 sm:p-6 sm:p-8">
            {activeTab === 'greeting' && (
                <div className="space-y-4 sm:space-y-6">
                  <div>
                    <h3 className="text-base sm:text-lg font-semibold text-white mb-1 sm:mb-2">Greeting Message</h3>
                    <p className="text-slate-400 mb-3 sm:mb-4 text-sm sm:text-base">This message will be played when a call starts</p>
                    <textarea
                    ...
                    />
                    <p className="text-xs sm:text-sm text-slate-500 mt-1 sm:mt-2">Character count: {(config.greeting_message || '').length}</p>
                  </div>
                </div>
            )}
            ...
            {activeTab === 'exit' && (
                <div className="space-y-4 sm:space-y-6">
                  <div>
                    <h3 className="text-base sm:text-lg font-semibold text-white mb-1 sm:mb-2">Exit Message</h3>
                    <p className="text-slate-400 mb-3 sm:mb-4 text-sm sm:text-base">This message will be played when a call ends</p>
                    <textarea
                    ...
                    />
                    <p className="text-xs sm:text-sm text-slate-500 mt-1 sm:mt-2">Character count: {(config.exit_message || '').length}</p>
                  </div>
                </div>
            )}
            ...
            {activeTab === 'behavior' && (
                <div className="space-y-4 sm:space-y-6">
                  <div>
                    <h3 className="text-base sm:text-lg font-semibold text-white mb-1 sm:mb-2">System Prompt Template</h3>
                    <p className="text-slate-400 mb-3 sm:mb-4 text-sm sm:text-base">Choose a template or customize your system prompt</p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-4 sm:mb-6">
                      ...
                      `}
                          >
                            <div className="font-medium text-white mb-1 text-sm sm:text-base">{template.name}</div>
                            <div className="text-xs sm:text-sm text-slate-400">{template.prompt.substring(0, 60)}...</div>
                          </button>
                      ))}
                    </div>
            ...
            {activeTab === 'knowledge' && (
                <div className="space-y-4 sm:space-y-6">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-0">
                    <div>
                      <h3 className="text-base sm:text-lg font-semibold text-white mb-1 sm:mb-2">Knowledge Base</h3>
                      <p className="text-slate-400 text-sm sm:text-base">Enable and configure custom knowledge for your agent</p>
                    </div>
                      ...
                      <div className="space-y-4 sm:space-y-6">
                        ...
                        <div>
                          ...
                          <textarea
                              ...
                              rows={6}
                              className="w-full px-3 py-2 sm:px-4 sm:py-3 bg-slate-800 border border-slate-700 rounded-lg sm:rounded-xl text-white placeholder-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all font-mono text-xs sm:text-sm"
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-between items-center gap-3 sm:gap-0">
          <button
              onClick={resetConfig}
              className="flex items-center space-x-2 px-4 py-2 sm:px-6 sm:py-3 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg sm:rounded-xl transition-all w-full sm:w-auto justify-center"
          >
          ...
          <button
              onClick={saveConfig}
              disabled={loading}
              className="flex items-center space-x-2 px-4 py-2 sm:px-8 sm:py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg sm:rounded-xl font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed w-full sm:w-auto justify-center"
          >
            <Save className="w-4 h-4" />
            <span className="text-sm sm:text-base">{loading ? 'Saving...' : 'Save Configuration'}</span>
          </button>
        </div>
