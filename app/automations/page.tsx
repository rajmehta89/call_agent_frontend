...
const actions = ['Create lead', 'Update lead', 'Send WhatsApp', 'Send Gmail email', 'Human handoff']
const leadStatuses = ['new', 'contacted', 'qualified', 'hot', 'converted', 'lost']

type Step = { type: string; message?: string; to?: string; subject?: string; body?: string }
...
export default function AutomationsPage() {
  ...
  const updateStep = (index: number, value: string) => setForm((current) => ({ ...current, steps: current.steps.map((step, stepIndex) => stepIndex === index ? { ...step, type: value, message: value === 'Send WhatsApp' ? step.message : undefined, to: value === 'Send Gmail email' ? step.to || '{{email}}' : undefined, subject: value === 'Send Gmail email' ? step.subject || 'Follow-up from Raj\'s team' : undefined, body: value === 'Send Gmail email' ? step.body || 'Hi {{name}},\n\nThanks for reaching out. I would be happy to learn about your current process and suggest an AI automation approach.\n\nBest,\nRaj\'s team' : undefined, status: value === 'Update lead' ? (step as Step & { status?: string }).status || 'contacted' : undefined } : step) }))
  ...
  return <div className="space-y-7">
    ...
    <DataState loading={loading} error={error} onRetry={load}>
      ...
      <div className="mt-5"><div className="flex items-center justify-between"><div className="text-xs font-semibold text-slate-600">Actions</div><button type="button" onClick={addStep} className="text-xs font-semibold text-[#5a67b1]">+ Add action</button></div><div className="mt-2 space-y-2">{form.steps.map((step, index) => <div key={index} className="flex flex-wrap items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-white text-xs font-bold text-slate-500">{index + 1}</span><select value={step.type} onChange={(event) => updateStep(index, event.target.value)} className="h-10 min-w-[220px] flex-1 rounded-lg border-slate-200 bg-white text-sm">{actions.map((action) => <option key={action}>{action}</option>)}</select>{step.type === 'Update lead' && <select value={(step as Step & { status?: string }).status || 'contacted'} onChange={(event) => setForm((current) => ({ ...current, steps: current.steps.map((item, stepIndex) => stepIndex === index ? { ...item, status: event.target.value } : item) }))} className="h-10 rounded-lg border-slate-200 bg-white text-sm">{leadStatuses.map((status) => <option key={status}>{status}</option>)}</select>}{step.type === 'Send WhatsApp' && <input value={step.message || ''} onChange={(event) => setForm((current) => ({ ...current, steps: current.steps.map((item, stepIndex) => stepIndex === index ? { ...item, message: event.target.value } : item) }))} placeholder="Message to send" className="h-10 min-w-[220px] flex-[2] rounded-lg border-slate-200 bg-white text-sm" />}{step.type === 'Send Gmail email' && <div className="grid min-w-[260px] flex-[2] gap-2 md:grid-cols-3"><input value={step.to || '{{email}}'} onChange={(event) => setForm((current) => ({ ...current, steps: current.steps.map((item, stepIndex) => stepIndex === index ? { ...item, to: event.target.value } : item) }))} placeholder="Recipient or {{email}}" className="h-10 rounded-lg border-slate-200 bg-white text-sm" /><input value={step.subject || ''} onChange={(event) => setForm((current) => ({ ...current, steps: current.steps.map((item, stepIndex) => stepIndex === index ? { ...item, subject: event.target.value } : item) }))} placeholder="Subject" className="h-10 rounded-lg border-slate-200 bg-white text-sm" /><textarea value={step.body || ''} onChange={(event) => setForm((current) => ({ ...current, steps: current.steps.map((item, stepIndex) => stepIndex === index ? { ...item, body: event.target.value } : item) }))} placeholder="Email body; use {{name}}, {{company}}, {{message}}" className="min-h-10 rounded-lg border-slate-200 bg-white text-sm md:col-span-3" /> </div>}{form.steps.length > 1 && <button type="button" onClick={() => removeStep(index)} aria-label="Remove action" className="rounded-lg p-2 text-slate-400 hover:bg-white hover:text-rose-500"><Trash2 className="h-4 w-4" /></button>}</div>)}</div></div>
