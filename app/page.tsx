...
import { Activity, ArrowUpRight, BarChart3, Bot, CalendarDays, CheckCircle2, ChevronRight, Clock3, Mail, MessageSquare, PhoneCall, Sparkles, Target, TrendingUp, Users, WalletCards } from 'lucide-react'
import { PageHeader } from '@/components/OmniPage'
import { API_BASE, DataState, StatusBadge, Toolbar } from '@/components/PlatformUI'

type DashboardPayload = { metrics?: any; channels?: any; shopify?: any; email?: any; recent_activity?: any[] }
  ...
  return <div className="space-y-6">
    ...
    <DataState loading={loading} error={error} onRetry={load}>
      ...
      <div className="grid gap-5 xl:grid-cols-[1.45fr_1fr]">
        ...
        <section className="surface-panel rounded-[22px] p-5"><SectionTitle icon={Bot} title="AI agent health" /><div className="mt-5 flex items-center gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#fff7ed] text-[#d97706]"><Bot className="h-5 w-5" /></div><div className="min-w-0 flex-1"><div className="text-sm font-bold text-slate-900">Shared AI assistant</div><div className="mt-1 text-xs text-slate-500">Knowledge and tools are shared across WhatsApp, Voice AI, and email automations.</div></div><StatusBadge value="Active" /></div><div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-4"><div className="rounded-xl border border-slate-200 p-3"><div className="text-[10px] uppercase tracking-wider text-slate-400">WhatsApp</div><div className="mt-2"><StatusBadge value={data.channels?.whatsapp?.connected ? 'Connected' : 'Not connected'} /></div></div><div className="rounded-xl border border-slate-200 p-3"><div className="text-[10px] uppercase tracking-wider text-slate-400">Voice AI agent</div><div className="mt-2"><StatusBadge value={data.channels?.voice?.connected ? 'Connected' : 'Not connected'} /></div></div><Link href="/brain/tools" className="rounded-xl border border-slate-200 p-3 transition hover:border-[#fdba74] hover:bg-[#fffaf4]"><div className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-slate-400"><Mail className="h-3 w-3" />Gmail automation</div><div className="mt-2"><StatusBadge value={data.email?.gmail?.connected ? 'Connected' : 'Needs setup'} /></div><div className="mt-1 truncate text-[10px] text-slate-500">{data.email?.gmail?.address || 'Connect sender account'}</div></Link><Link href="/campaigns" className="rounded-xl border border-slate-200 p-3 transition hover:border-[#a7ddbd] hover:bg-[#f8fcfa]"><div className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-slate-400"><Target className="h-3 w-3" />Email campaign</div><div className="mt-2"><StatusBadge value={data.email?.campaign?.status || 'Stopped'} /></div><div className="mt-1 text-[10px] text-slate-500">{Number(data.email?.outbox?.pending_approval || 0)} awaiting · {Number(data.email?.outbox?.sent || 0)} sent</div><div className="mt-1 text-[10px] text-slate-400">{Number(data.email?.campaign?.rate_used || 0)} / {Number(data.email?.campaign?.max_emails || 20)} in rate window</div></Link></div><div className="mt-4 rounded-xl border border-[#fed7aa] bg-[#fff7ed] p-4"><div className="flex items-start gap-3"><Users className="mt-0.5 h-4 w-4 shrink-0 text-[#d97706]" /><div><div className="text-sm font-semibold text-slate-800">Need help with your AI agent?</div><p className="mt-1 text-xs leading-5 text-slate-500">Review shared knowledge, tools, and integrations when a conversation needs attention.</p><Link href="/brain/knowledge" className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-[#b45309] hover:underline">Open assistant settings <ChevronRight className="h-3.5 w-3.5" /></Link></div></div></div></section>
      </div>
    </DataState>
  </div>
}
