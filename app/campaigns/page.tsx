...
type Campaign = { status: 'running' | 'paused' | 'stopped'; max_emails: number; interval_minutes: number; rate_used: number; rate_remaining: number; can_process: boolean; discovery_enabled?: boolean; discovery_query?: string; discovery_location?: string; approval_window_open?: boolean; send_window_open?: boolean }
...
const defaultCampaign: Campaign = { status: 'stopped', max_emails: 20, interval_minutes: 60, rate_used: 0, rate_remaining: 20, can_process: false, discovery_enabled: true, discovery_query: 'AI automation for home services', discovery_location: 'United States' }
...
export default function CampaignsPage() {
  ...
  const saveCampaign = async (next: Campaign) => {
    try {
      const result = await api<any>('/api/platform/email-campaign', { method: 'PUT', body: JSON.stringify({ value: { max_emails: next.max_emails, interval_minutes: next.interval_minutes, discovery_enabled: next.discovery_enabled, discovery_query: next.discovery_query, discovery_location: next.discovery_location } }) })
