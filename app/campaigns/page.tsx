...
type Campaign = { status: 'running' | 'paused' | 'stopped'; max_emails: number; interval_minutes: number; daily_limit: number; rate_used: number; rate_remaining: number; daily_used: number; daily_remaining: number; daily_limit_reached?: boolean; daily_reset_at?: string; can_process: boolean; discovery_enabled?: boolean; discovery_query?: string; discovery_location?: string; approval_window_open?: boolean; send_window_open?: boolean }
...
const defaultCampaign: Campaign = { status: 'stopped', max_emails: 20, interval_minutes: 60, daily_limit: 450, rate_used: 0, rate_remaining: 20, daily_used: 0, daily_remaining: 450, can_process: false, discovery_enabled: true, discovery_query: 'AI automation for home services', discovery_location: 'United States' }
...
export default function CampaignsPage() {
  ...
  const saveCampaign = async (next: Campaign) => {
    try {
      const result = await api<any>('/api/platform/email-campaign', { method: 'PUT', body: JSON.stringify({ value: { max_emails: next.max_emails, interval_minutes: next.interval_minutes, daily_limit: next.daily_limit, discovery_enabled: next.discovery_enabled, discovery_query: next.discovery_query, discovery_location: next.discovery_location } }) })
    ...
    <DataState loading={loading} error={error} onRetry={load}>
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
        <section className="surface-panel rounded-[24px] p-6">
          ...
          <div className="mt-4 rounded-xl border border-[#d9def7] bg-[#f8f9ff] px-4 py-3 text-xs leading-5 text-slate-600"><span className="font-semibold text-slate-800">Review flow:</span> new approval batches are automatically emailed to {gmail.address || 'your Gmail'}; approve or exclude them here. The review email never contacts prospects.</div>
        ...
        <section className="surface-panel rounded-[24px] p-6">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4"><div><div className="text-[10px] uppercase tracking-wider text-slate-400">Hourly window</div><div className="mt-1 text-lg font-bold text-slate-800">{campaign.rate_used} / {campaign.max_emails}</div></div><div><div className="text-[10px] uppercase tracking-wider text-slate-400">Today (IST)</div><div className="mt-1 text-lg font-bold text-slate-800">{campaign.daily_used} / {campaign.daily_limit}</div></div></div>
          <div className="mt-4 grid gap-3 sm:grid-cols-3"><label className="text-xs font-semibold text-slate-600">Emails per interval<input type="number" min="1" max="10000" value={campaign.max_emails} onChange={(event) => setCampaign({ ...campaign, max_emails: Number(event.target.value) })} onBlur={() => saveCampaign(campaign)} className="mt-2 h-10 w-full rounded-lg border-slate-200 text-sm" /></label><label className="text-xs font-semibold text-slate-600">Interval in minutes<input type="number" min="1" max="10080" value={campaign.interval_minutes} onChange={(event) => setCampaign({ ...campaign, interval_minutes: Number(event.target.value) })} onBlur={() => saveCampaign(campaign)} className="mt-2 h-10 w-full rounded-lg border-slate-200 text-sm" /></label><label className="text-xs font-semibold text-slate-600">Free daily limit<input type="number" min="1" max="500" value={campaign.daily_limit} onChange={(event) => setCampaign({ ...campaign, daily_limit: Number(event.target.value) })} onBlur={() => saveCampaign(campaign)} className="mt-2 h-10 w-full rounded-lg border-slate-200 text-sm" /></label></div>
          <div className={`mt-3 rounded-xl border p-3 text-xs leading-5 ${campaign.daily_limit_reached ? 'border-amber-200 bg-amber-50 text-amber-800' : 'border-emerald-100 bg-emerald-50 text-emerald-800'}`}>{campaign.daily_limit_reached ? 'Daily free Gmail limit reached. The campaign will remain ready and automatically continue after the next IST day starts.' : `Daily protection is active: ${campaign.daily_remaining} emails remain. Set the daily limit below 500 to leave room for normal Gmail use.`}</div>
          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4"><label className="flex items-center gap-3 text-xs font-semibold text-slate-700"><input type="checkbox" checked={campaign.discovery_enabled !== false} onChange={(event) => { const next = { ...campaign, discovery_enabled: event.target.checked }; setCampaign(next); saveCampaign(next) }} />Automatically refill the queue with unique prospects until this interval's target is ready</label><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-xs font-semibold text-slate-600">Business search<input value={campaign.discovery_query || ''} onChange={(event) => setCampaign({ ...campaign, discovery_query: event.target.value })} onBlur={() => saveCampaign(campaign)} placeholder="AI automation for home services" className="mt-2 h-10 w-full rounded-lg border-slate-200 bg-white text-sm" /></label><label className="text-xs font-semibold text-slate-600">Location<input value={campaign.discovery_location || ''} onChange={(event) => setCampaign({ ...campaign, discovery_location: event.target.value })} onBlur={() => saveCampaign(campaign)} placeholder="United States" className="mt-2 h-10 w-full rounded-lg border-slate-200 bg-white text-sm" /></label></div><p className="mt-2 text-[11px] leading-5 text-slate-500">Duplicate businesses and email addresses are skipped permanently, including previously excluded prospects.</p></div>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2"><label className="text-xs font-semibold text-slate-600">Approval starts<input type="text" inputMode="numeric" maxLength={5} placeholder="00:00" value={policy.approval_start_time} onChange={(event) => setPolicy({ ...policy, approval_start_time: event.target.value })} onBlur={() => savePolicy(policy)} className="mt-2 h-10 w-full rounded-lg border-slate-200 text-sm" /></label><label className="text-xs font-semibold text-slate-600">Approval ends<input type="text" inputMode="numeric" maxLength={5} placeholder="24:00" value={policy.approval_end_time} onChange={(event) => setPolicy({ ...policy, approval_end_time: event.target.value })} onBlur={() => savePolicy(policy)} className="mt-2 h-10 w-full rounded-lg border-slate-200 text-sm" /></label><label className="text-xs font-semibold text-slate-600">Send starts<input type="text" inputMode="numeric" maxLength={5} placeholder="00:00" value={policy.send_start_time} onChange={(event) => setPolicy({ ...policy, send_start_time: event.target.value })} onBlur={() => savePolicy(policy)} className="mt-2 h-10 w-full rounded-lg border-slate-200 text-sm" /></label><label className="text-xs font-semibold text-slate-600">Send ends<input type="text" inputMode="numeric" maxLength={5} placeholder="24:00" value={policy.send_end_time} onChange={(event) => setPolicy({ ...policy, send_end_time: event.target.value })} onBlur={() => savePolicy(policy)} className="mt-2 h-10 w-full rounded-lg border-slate-200 text-sm" /></label></div>
