...
export const COUNTRY_CODE_OPTIONS: CountryCodeOption[] = COUNTRY_CODE_DATA.split(',')
  .map((item) => {
    const [region, digits] = item.split(':')
    return {
      region,
      code: `+${digits}`,
      name: nameOverrides[region] || regionNames?.of(region) || region,
    }
  })
