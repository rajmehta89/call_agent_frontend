...
import { ArrowUpRight, Brain, Mail, MessageSquare, PhoneCall, ShoppingBag } from 'lucide-react'
...
const catalog = [
  ...
  { name: 'Gmail', category: 'Email automation', description: 'Send opt-in lead follow-ups and customer emails from the connected Gmail account.', icon: Mail, href: '/brain/tools' },
]
...
export default function IntegrationsPage() {
  ...
  useEffect(() => { load() }, [])
  const status = (name: string) => name === 'WhatsApp' ? data.dashboard?.channels?.whatsapp?.connected : name === 'Twilio Voice' ? data.dashboard?.channels?.voice?.connected : name === 'OpenAI' ? data.summary?.ai?.enabled : name === 'Gmail' ? data.summary?.email?.gmail_connected : data.dashboard?.shopify?.connected
